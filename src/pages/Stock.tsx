...
const Stock = () => {
	...
	const filteredMotorcycles = motorcyclesData.filter((bike) => {
		...
		return (
			matchesSearch && matchesBrand && matchesType && matchesColor && matchesPatio && matchesStatus
		);
	});
			...
			<div className='space-y-6'>
				...
				<div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
					<div>
						...
						<p className='text-muted-foreground'>Consulta de motos novas e seminovas disponíveis</p>
					</div>
				...
				<div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
					<div className='card-elevated p-4'>
						...
						<p className='text-2xl font-bold text-foreground'>
							{filteredMotorcycles.reduce((sum, bike) => sum + bike.stock, 0)}
						</p>
					</div>
					<div className='card-elevated p-4'>
					...
					<div className='card-elevated p-4'>
						...
						<p className='text-2xl font-bold text-success'>
							{filteredMotorcycles
								.filter((bike) => bike.type === 'Nova')
								.reduce((sum, bike) => sum + bike.stock, 0)}
						</p>
					</div>
					<div className='card-elevated p-4'>
						...
						<p className='text-2xl font-bold text-warning'>
							{filteredMotorcycles
								.filter((bike) => bike.type === 'Seminova')
								.reduce((sum, bike) => sum + bike.stock, 0)}
						</p>
					</div>
				</div>
						...
						<TableBody>
							{filteredMotorcycles.map((motorcycle) => (
								<TableRow key={motorcycle.id}>
									...
									<TableCell className='font-medium'>{formatPrice(motorcycle.price)}</TableCell>
									<TableCell>
										<span
											className={`font-medium ${
												motorcycle.stock < 5 ? 'text-error' : 'text-foreground'
											}`}
										>
											{motorcycle.stock}
										</span>
